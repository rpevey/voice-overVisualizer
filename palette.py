"""
palette.py
Shared color handling for both visualizers. Per project constraint, every
color in this toolkit is either a literal RGBA value or a sample from one of
two colormaps: viridis or greyscale. No arbitrary hue-cycling anywhere.

Color spec syntax (used for --bg-color, --bar-color, etc.):
  "#RRGGBB"          opaque hex
  "#RRGGBBAA"        hex with alpha
  "viridis:T"        sample viridis at T in [0, 1], alpha=255
  "viridis:T:AA"     same, with 2-digit hex alpha
  "grey:V"           sample greyscale at V in [0, 1] (equivalent to viridis
                     but flat luminance), alpha=255
  "grey:V:AA"        same, with alpha
"""
import numpy as np

# Embedded 256-entry viridis LUT (extracted from matplotlib) so the toolkit
# has zero dependency on matplotlib at runtime -- just numpy/scipy/Pillow.
...
# Simple linear greyscale LUT: value V maps to (V, V, V).
GREYSCALE_LUT = np.array([(i, i, i) for i in range(256)], dtype=np.uint8)

PALETTES = {
    "viridis": VIRIDIS_LUT,
    "greyscale": GREYSCALE_LUT,
    "grey": GREYSCALE_LUT,
}


def sample_palette(t, palette="viridis", alpha=255):
    """Sample a palette at t in [0, 1]. Returns an (r, g, b, a) int tuple."""
    lut = PALETTES[palette]
...
def parse_color(spec, default_palette="viridis"):
    """
    Parse a color spec string into an (r, g, b, a) int tuple.
    Accepts "#RRGGBB", "#RRGGBBAA", "viridis:T[:AA]", "grey:V[:AA]".
    """
    if isinstance(spec, (tuple, list)):
        if len(spec) == 3:
            return (int(spec[0]), int(spec[1]), int(spec[2]), 255)
    ...
    if s.startswith("#"):
        ...
        if len(hexs) == 6:
            ...
            r, g, b, a = (int(hexs[0:2], 16), int(hexs[2:4], 16),
                          int(hexs[4:6], 16), int(hexs[6:8], 16))
            return (r, g, b, a)
        raise ValueError(f"Unrecognized hex color: {spec}")

    if ":" in s:
        parts = s.split(":")
        palette_name = parts[0].lower()
        if palette_name not in PALETTES:
            raise ValueError(f"Unknown palette '{palette_name}'. Use 'viridis' or 'grey'.")
        t = float(parts[1])
        alpha = int(parts[2], 16) if len(parts) > 2 else 255
        return sample_palette(t, palette=palette_name, alpha=alpha)

    raise ValueError(
        f"Unrecognized color spec: '{spec}'. Use '#RRGGBB[AA]', 'viridis:T', or 'grey:V'."
    )
